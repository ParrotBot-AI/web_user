<template>
  <div>
    <h2 class="text-gray-500 text-[16px] pb-3 font-bold">
      ...
      ideas that are not presented in the passage or are minor ideas in the passage. This questions
      is worth 2 points. Drag your answer choices to the spaces here they belong. To remove an
    ...
    <div>
      <DropBox
        ...
        :onDrop="onDrop"
        :onDel="onDel"
        :accept="props.options_label.map((val) => val)"
        :index="i"
      ></DropBox>
    </div>
...
const onDrop = ({ type, index }: { type: string; index: number }) => {
  ...
  if (count === props.restriction.rc) {
    examStore.saveQuestion(
      props.question_id,
      props.options_label.map((val) => (res.value.includes(val) ? 1 : 0))
    )
  }
}
