  ...
  <a-layout class="w-full h-full flex flex-col">
    ...
    >
      <template #right>
        <BQuesTimer
          ...
          :times="examStore?.questionData?.time_remain"
        />
        <div
          v-show="examStore.curQuestion?.viewText_open"
        ...
        >
          {{ !examStore.curQuestion?.viewText_flag ? 'VIEW QUESTION' : 'VIEW TEXT' }}
        </div>
      </template>
    </BQuesTitle>
    <div class="flex flex-1 overflow-hidden bg-white w-full">
        ...
        <div ref="contentDiv" id="content" class="content-box">
          <p
            ...
            :class="[
              ...
              {
                active:
                  typeof examStore.curQuestion?.keywords?.p === 'number'
                    ? examStore.curQuestion?.keywords?.p === i + 1
                    : examStore.curQuestion?.keywords?.p?.[0] === i + 1
              }
            ]"
            v-for="(val, i) in examStore.curQuestion?.parentQuestion?.question_content"
            v-html="val"
            :key="i"
          ></p>
        </div>
      ...
      <div
        ...
        :class="{
          'w-full opacity-1':
            examStore.curQuestion?.viewText_open && examStore.curQuestion?.viewText_flag,
...
watchEffect(() => {
  if (
    examStore.questionData?.sumQuesLength > 0 &&
    examStore.curQuestion?.num === examStore.questionData?.sumQuesLength
  ) {
      ...
      setTimeout(() => {
        ...
        if (spanTarget) {
          examStore.saveQuestion(
            examStore.curQuestion?.question_id,
            examStore.curQuestion?.options_label.map((val: any, i: any) =>
              i == spanTarget?.dataset.index ? 1 : 0
            )
          )
        }
      })
