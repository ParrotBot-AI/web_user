<template>
  <div class="h-full w-full fixed top-0 left-0 z-[100] bg-[#edf6f6]" v-show="showHelp">
    <b-header :title="$t('单词练习')" :onClickBack="onClickHelp">
      <template #right>
        <div class="flex items-center">
          <a-button type="primary" class="mx-2 flex items-center justify-center h-[40px]" @click="onClickHelp"><img
              :src="left" class="pr-2" />{{ $t('返回') }}</a-button>
        </div>
      </template>
    </b-header>
    <div class="help-container">
      ...
      <p class="text-[#667085]">{{
        $t('鹦鹉智学AI单词背诵会使用生词用AI生成文章来辅助背诵。这种背诵方式功能会让你的背诵记忆更加牢固、快速，也更加适应托福考试的要求。在我们过去做的实验中，100%的学生认为背诵效果提升，100%的学生认为这让单词背诵变得有趣。')
        }}</p>
  ...
  <div class="h-full w-full flex flex-col">
      ...
      <template #right>
        <div class="flex items-center">
          <p class="text-xs text-[#1B8B8C] mr-3 flex" v-if="curTask?.payload?.word && !curTask?.payload?.after_model">
            <span class="mx-2 flex items-center"><img :src="icon1" class="mx-1" />{{ $t('已学习单词') }} {{
              curTask.payload?.today_study || 0}}</span>
            <span class="flex items-center"><img :src="icon2" class="mx-1" />{{ $t('发现生词') }} {{
              curTask.payload?.process?.c || 0}}/{{ curTask.payload?.process?.t || 0 }}</span>
          </p>
          ...
          <a-button type="primary" class="mx-2 flex items-center justify-center h-[40px]" @click="onClickHelp"><img
              :src="help" class="pr-2" />{{ $t('帮助') }}</a-button>
          <a-button type="primary" class="mx-2 flex items-center justify-center h-[40px]"
            @click="wordStore.submit_Study()" v-if="curTask?.payload?.word && !curTask?.payload?.after_model">{{
              $t('直接进入背诵') }} <img :src="right" class="pl-2" /></a-button>
        </div>
      </template>
    </b-header>
    <div class="bg-[#edf6f6] flex flex-1 overflow-hidden" v-if="!wordStore.finished">
      <a-spin v-if="loading" size="large" tip="试题加载中..."
        class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50" />
      <template v-else>
        <!--单词背诵-->
        <div class="flex w-full items-center flex-col bg-[#edf6f6] pt-44" :class="{ '!pt-20': curTask?.payload?.hint }"
          v-if="curTask?.payload?.word">
          <p v-if="curTask?.payload?.hint" class="text-[#667085] text-[18px] font-medium pb-5 w-[500px]"> {{
            curTask?.payload?.hint }} </p>
          <h2 class="pb-20">{{ curTask?.payload?.word }}{{ $t('的意思是？') }}</h2>
          <div class="grid grid-cols-2 gap-8">
            <div v-for="(val, i) in curTask?.payload?.stem" :key="i" class="w-[220px] wordBlock p-2" :class="{
          ...
          <a-button type="primary" class="mt-10 w-[270px] h-11" @click="wordStore.error_next()"
            v-if="curTask?.is_error">{{ $t('下一步') }}</a-button>
          <a-button type="primary" class="mt-10 w-[270px] h-11" @click="wordStore.submit_unknown()" v-else>{{
            $t('不认识该词') }}</a-button>
        </div>
        <!--AI文章-->
        <div class="flex w-full h-full items-center flex-col bg-[#edf6f6] md:px-40 px-12 py-14" v-else>
          <div class="w-full h-[calc(100%-60px)] max-w-[1200px] relative bg-wrap flex flex-col items-center">
            <div class="bg-white rounded-md w-full h-[98%] relative z-20 pt-20 pb-4 shrink-0"
              :style="{ boxShadow: '0 4px 20px rgba(27, 139, 140, 0.2)' }">
              <h3
                class="inline-block bg-[rgba(243,184,78,0.2)] rounded-tr-xl rounded-br-xl text-[#C79741] px-4 py-1 text-base font-normal absolute left-0 top-3">
                Genrated by AI</h3>
              <BAutoWord :data="curTask?.payload?.response!" />
            </div>
            <a-button type="primary" class="w-[300px] h-[44px] shrink-0 mt-6" @click="onClickAINext"
              v-if="curTask?.is_end">{{ $t('下一步') }}</a-button>
          </div>
        </div>
      </template>
    </div>
    <div class="bg-[#edf6f6] flex flex-1 overflow-hidden items-center justify-center" v-else>
      <div class="flex flex-col items-center justify-center h-full">
        ...
        <a-button type="primary" class="mt-10 w-[300px] h-[44px]" @click="router.push('/wordRecite')">{{ $t('返回单词首页')
          }}</a-button>
      </div>
    </div>
  </div>
