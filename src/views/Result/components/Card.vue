<template>
  <div class="bg-white rounded-md px-14 pb-14 pt-24 relative" :style="{boxShadow: `0px 4px 20px 0px rgba(27, 139, 140, 0.20)`}">
    ...
    <span class="absolute right-14 top-6 cursor-pointer text-[#F7A705]">回顾答案<ArrowRightOutlined class="pl-2"/></span>
    <div class="w-[56vw]">
      <ul class="flex mb-8 mt-2 list">
        <li v-for="val in summaryList" :key="val.id" class="flex-1 list-none flex items-center justify-center flex-col">
          ...
          <p class="font-bold text-gray-600">
            <span class="text-[36px]">{{ props[val.id] }}</span><span class="text-[24px]"> / {{ props[`${val.id}Total`] }}</span>
          </p>
        </li>
      </ul>
      <div class="flex flex-wrap">
        <span v-for="(val,i) in props.questions" :key="i" class="px-4 py-1.5 rounded-full text-gray-600 bg-[#E7EAEE] text-[16px] mx-2">
