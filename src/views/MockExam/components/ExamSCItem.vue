<template>
  <div>
    ...
    <a-radio-group v-model:value="sc_value"
      v-if="props.restriction_count === 1 && props.keywords.k !== '$$' && props.question_type !== 'TR_fill_sentence'">
      <a-radio v-for="(item, index) in props.choice" :key="index" :value="index" class="flex pb-7 text-gray-500 myraido">
        <span class="pl-3 pr-2">{{ props.choice_label[index] }}.</span>
        <p class="flex-1">{{ item }}</p>
      </a-radio>
    </a-radio-group>
    <a-checkbox-group v-model:value="mc_value" v-else-if="props.restriction_count > 1" class="flex flex-col">
      <a-checkbox v-for="(item, index) in props.choice" :key="index" :value="index"
        class="flex pb-7 text-gray-500 mycheckbox flex-row">
        <span class="pl-3 pr-2">{{ props.choice_label[index] }}.</span>
        <p class="flex-1 overflow-hidden text-wrap">{{ item }}</p>
      </a-checkbox>
    </a-checkbox-group>
    <h2 v-else-if="props.keywords.k === '$$' || props.question_type === 'TR_fill_sentence'"
...
watch(() => mc_value.value, () => {
  const value = props.choice.map((val, i) => Number((Object.values(mc_value.value) as number[]).includes(i)))
  examStore.saveQuestion(props.question_id, value)
})
</script>
<style scoped>
