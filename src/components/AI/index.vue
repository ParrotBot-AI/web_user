<template>
  <div class="flex flex-col w-[280px] lg:w-[340px] bg-white rounded-md border border-border-1 border-solid shadow-lg">
    <header class="flex px-3 pt-4 items-center justify-center">
      ...
      <h2 class="flex-1 pl-4 font-normal text-green-3 py-2.5"
        :style="{ borderBottom: `1px solid var(--color-gray-200)` }">{{ $t('鹦鹉AI助教') }}</h2>
    </header>
    ...
    <footer class="flex px-6 py-4" :style="{ borderTop: `1px solid var(--color-gray-300)` }">
      <a-input class="flex-1 mr-3 h-10" :placeholder="$t('输入消息')" v-model:value="val" @keyup.enter="onSend" />
